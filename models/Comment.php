...
class Comment
{
    ...
    public static function getArticleById($id)
    {
        ...
        if($id) {
            ...
            $result = $db->query("SELECT title FROM article WHERE id=" . $id);
            $result->setFetchMode(PDO::FETCH_ASSOC);
    ...
    public static function getCategoryByArticleId($id)
    {
        ...
        if($id) {
            ...
            $result2 = $db->query("SELECT title FROM category WHERE id=" . $result['category_id']);
    ...
    public static function createComment($options)
    {
        ...
        if(self::getCategoryByArticleId($options['article_id']) == 'Политика'){
